...
def save_code_cfg(cfg, ckpt_dir):
    ...
    torch.save({
        'cfg': cfg,
        'code': code_files
    }, Path(ckpt_dir).joinpath('code_cfg.tar').absolute().as_posix())

class VisCallbackAE(Callback):
    @pl.utilities.rank_zero_only
    def on_train_epoch_end(self, trainer, pl_module):
        if trainer.current_epoch % pl_module.cfg.AE.log_per_epochs == 0:
            print(pl_module.device)
            pl_module = pl_module.eval()
            renorm_transform = trainer.datamodule.renorm_transform

            dataloader = trainer.datamodule.train_dataloader()
            save_dir = Path(pl_module.cfg.AE.ckpt_save_dir).joinpath(f'train_gifs_epoch{trainer.current_epoch}')
            self.vis_samples(dataloader, renorm_transform, pl_module, save_dir)

            dataloader = trainer.datamodule.val_dataloader()
            save_dir = Path(pl_module.cfg.AE.ckpt_save_dir).joinpath(f'val_gifs_epoch{trainer.current_epoch}')
            self.vis_samples(dataloader, renorm_transform, pl_module, save_dir)

    def vis_samples(self, dataloader, renorm_transform, pl_module, save_dir):
        past_frames, future_frames = next(iter(dataloader))
        past_frames, future_frames = past_frames.to(pl_module.device), future_frames.to(pl_module.device)
        with torch.no_grad():
            rec_past_frames, _ = pl_module(past_frames)
            rec_future_frames, _ = pl_module(future_frames)
            
            gt_frames = torch.cat([past_frames, future_frames], dim = 1)
            rec_frames = torch.cat([rec_past_frames, rec_future_frames], dim = 1)
            
        N = future_frames.shape[0]
        idx = min(N, 4)

        visualize_batch_clips(gt_frames[0:idx, :, ...], rec_frames[0:idx, :, ...], rec_frames[0:idx, :, ...], save_dir, renorm_transform, desc = 'ae')

class VisCallbackPredictor(Callback):
    @pl.utilities.rank_zero_only
    def on_train_epoch_end(self, trainer, pl_module):
        if trainer.current_epoch % pl_module.cfg.Predictor.log_per_epochs == 0:
            pl_module = pl_module.eval()
            renorm_transform = trainer.datamodule.renorm_transform

            dataloader = trainer.datamodule.train_dataloader()
            save_dir = Path(pl_module.cfg.Predictor.ckpt_save_dir).joinpath(f'train_gifs_epoch{trainer.current_epoch}')
            self.vis_samples(dataloader, renorm_transform, pl_module, save_dir)

            dataloader = trainer.datamodule.val_dataloader()
            save_dir = Path(pl_module.cfg.Predictor.ckpt_save_dir).joinpath(f'val_gifs_epoch{trainer.current_epoch}')
            self.vis_samples(dataloader, renorm_transform, pl_module, save_dir)

    def vis_samples(self, dataloader, renorm_transform, pl_module, save_dir):
        batch = next(iter(dataloader))
        batch = pl_module.batch_process_fn(batch)
        past_frames, future_frames = batch
        past_frames, future_frames = past_frames.to(pl_module.device), future_frames.to(pl_module.device)
        with torch.no_grad():
            rec_past_frames, rec_future_frames, pred_future_frames = pl_module(past_frames, future_frames)

        N = pred_future_frames.shape[0]
        idx = min(N, 4)

        visualize_batch_clips(past_frames[0:idx, :, ...], rec_past_frames[0:idx, :, ...], rec_future_frames[0:idx, :, ...], save_dir, renorm_transform, desc = 'ae')
...
def resume_training(module_dict, optimizer_dict, resume_ckpt, loss_name_list = None, map_location = None):
    ...
    if map_location is None:
        ...
        return history_loss_dict, start_epoch
...
def save_ckpt(Modules_dict, Optimizers_dict, epoch, loss_dict, ckpt_codes, save_dir):
...
def show_AE_samples(VPTR_Enc, VPTR_Dec, sample, renorm_transform, device, save_dir):
...
def show_predictor_samples(VPTR_Enc, VPTR_Dec, predictor, sample, save_dir, device, renorm_transform):
    ...
    with torch.no_grad():
        ...
        rec_future_frames = VPTR_Dec(future_gt_feats)
        try:
            stochastic_flag = predictor.stochastic
        except AttributeError as err:
            stochastic_flag = predictor.module.stochastic
        if stochastic_flag:
            pred_future_feats, _, _, _, _ = predictor(past_gt_feats, future_gt_feats)
        else:
            pred_future_feats = predictor(past_gt_feats, future_gt_feats)
...
if __name__ == '__main__':
    #code_file_dict = read_code_files()
    modules_state_dict, optimizers_state_dict, start_epoch, history_loss_dict, code_file_dict = load_ckpt('/home/travail/xiyex/VPTR_ckpts/MNIST_GPT_MSEGDL_ckpt/epoch_193.tar')
    write_code_files(code_file_dict, '/home/travail/xiyex/Copy1VideoFramePrediction')
